...
// Vietnamese trigram names for display
...
// Short quẻ name: last word of full name
function shortQ(hex){if(!hex)return'';const parts=hex[1].split(' ');return parts[parts.length-1]}
...
export default function App(){
  ...
  const onTS=useCallback(e=>{touchX.current=e.touches[0].clientX},[]);
  const onTE=useCallback(e=>{
    const dx=e.changedTouches[0].clientX-touchX.current;
    if(dx>80&&touchX.current<60){// swipe right from left edge
      if(view==='result'||view==='dacbiet'||view==='nhap'||view==='ngaunhien'){setView('home');setResult(null);setLuanResult('');setChatHistory([])}
      else if(view==='question'){setView('home')}
    }
  },[view]);

  // Pull-down state for ngẫu nhiên
  const[pullY,setPullY]=useState(0);
  const pullStart=useRef(0);
  const onPullTS=useCallback(e=>{pullStart.current=e.touches[0].clientY},[]);
  const onPullTM=useCallback(e=>{const dy=e.touches[0].clientY-pullStart.current;if(dy>0&&window.scrollY===0)setPullY(Math.min(dy*.4,80))},[]);
  const onPullTE=useCallback(()=>{if(pullY>50){castNgauNhien()}setPullY(0)},[pullY]);
  ...
  const finish=(r,method,question='',name='')=>{
    ...
    return{id:Date.now(),...r,queHo:ho,method,question,name,lunar:lu,ts:nowTS()};
  };
  const addHistory=(full)=>{setHistory(prev=>{const u=[full,...prev].slice(0,50);localStorage.setItem('kd_history',JSON.stringify(u));return u})};

  const cast=(method)=>{
    const now=new Date(),lu=s2l(now.getDate(),now.getMonth()+1,now.getFullYear()),hi=hIdx(now.getHours())+1;
    let r;
    if(method==='thoi'){const us=lu.year+lu.month+lu.day,ls=us+hi;r=finish({...mhCalc(us,ls,ls)},'Thời')}
    else if(method==='khac'){const us=lu.year+lu.month+lu.day+hi,ls=us+(now.getMinutes()+1);r=finish({...mhCalc(us,ls,ls)},'Khắc')}
    else if(method==='giay'){const sec=now.getSeconds()+1,us=lu.year+lu.month+lu.day+hi;r=finish({...mhCalc(us,us+sec,us+sec+now.getMilliseconds())},'Giây')}
    else if(method==='dacbiet'){
      const nums=specialNum.replace(/\D/g,'');if(!nums){alert('Nhập số');return null}
      ...
      r=finish({...mhCalc(u,l,total||1)},'Đặc Biệt');
    }
    if(r){setResult(r);addHistory(r);setLuanResult('');setChatHistory('');setPhamVi('');setView('result')}
    return r;
  };
  ...
  const castNgauNhien=()=>{
    ...
    const r=finish({chinh:getHexagram(lv),bien:moving.length>0?getBienQue(lv,moving):null,lines,lineValues:lv,moving},'Ngẫu Nhiên');
    setResult(r);addHistory(r);setLuanResult('');setChatHistory([]);setPhamVi('');
    setView('ngaunhien');
  };
  ...
  const castNhap=()=>{
    ...
    const r=finish({chinh:getHexagram(lv),bien:manualMoving.length>0?getBienQue(lv,manualMoving):null,lines,lineValues:lv,moving:manualMoving},'Nhập Quẻ');
    setResult(r);addHistory(r);setLuanResult('');setChatHistory([]);setPhamVi('');setView('result');
  };
  ...
  // ======== HEX RENDER — equal width, bigger gap ========
  const RHex=({lv,moving=[],w=90})=>{
    const h=Math.max(7,w/11);const gap=Math.max(6,w/10);const half=(w-w*.12)/2;
    return <div style={{display:'flex',flexDirection:'column-reverse',alignItems:'center',gap}}>{lv.map((v,i)=>{
      const isM=moving.includes(i);const cl=isM?P.red:P.fg;
      return <div key={i} style={{width:w,display:'flex',justifyContent:v===1?'center':'space-between',position:'relative'}}>
        ...
        {isM&&<span style={{position:'absolute',right:-14,top:-1,fontSize:9,color:P.red}}>●</span>}
      </div>})}</div>;
  };

  // Quẻ block: Thượng name / Hạ name / Quẻ short name
  const QueBlock=({hex,lv,mv=[],label,w=80})=>{
    if(!hex)return null;
    const upper=VIET_NAME[hex[3]]||'',lower=VIET_NAME[hex[4]]||'';
    return <div style={{textAlign:'center',cursor:'pointer',flex:1}} onClick={()=>setPopup(hex)}>
      ...
      <div style={{marginTop:10,fontSize:13,color:P.fg,lineHeight:1.3}}>{upper}</div>
      <div style={{fontSize:13,color:P.fg}}>{lower}</div>
      <div style={{fontSize:16,fontWeight:700,color:P.accent,marginTop:2}}>{shortQ(hex)}</div>
    </div>;
  };
      ...
      <div style={wrap} onTouchStart={onTS} onTouchEnd={onTE}>
        <div style={{maxWidth:480,margin:'0 auto',padding:'20px 16px'}}>
          ...
          <button onClick={()=>{setQName('');setQText('');setQResult(null);setView('question')}}
            ...
            <div>
              <div style={{fontSize:15,fontWeight:600,color:P.accent}}>Đặt câu hỏi gieo quẻ</div>
              <div style={{fontSize:11,color:P.muted,marginTop:2}}>Nhập sự việc → gieo → lưu → luận giải AI</div>
            </div>
          </button>

          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:10}}>
            <MB icon="◴" label="Thời" color={P.red} onClick={()=>cast('thoi')}/>
            <MB icon="◷" label="Khắc" color={P.amber} onClick={()=>cast('khac')}/>
            <MB icon="◎" label="Giây" color={P.teal} onClick={()=>cast('giay')}/>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
            ...
            <MB icon="✦" label="Đặc Biệt" color={P.blue} onClick={()=>setView('dacbiet')}/>
          </div>
        </div>
        <Settings2/><Popup2/>
      </div>
  ...
  // ======== QUESTION PAGE (with save + history) ========
  if(view==='question'){
    const r=qResult;
    return(
      <div style={wrap} onTouchStart={onTS} onTouchEnd={onTE}>
        <div style={{maxWidth:480,margin:'0 auto',padding:'16px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            <button onClick={()=>setView('home')} style={{background:'none',border:'none',color:P.accent,fontSize:13,cursor:'pointer',fontWeight:600}}>← Quay lại</button>
          ...
          {!r&&<div style={{textAlign:'center',padding:'20px 0'}}>
            ...
            <div onClick={castQuestion}
              style={{width:140,height:140,borderRadius:'50%',background:`radial-gradient(circle at 40% 40%, ${P.accentBg}, ${P.accent})`,display:'inline-flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:`0 6px 24px ${P.accent}30`,fontSize:56,color:'#fff',userSelect:'none'}}>☯</div>
          </div>}
          ...
          {r&&<>
            ...
            <div style={{display:'flex',justifyContent:'space-evenly',padding:'24px 8px',marginBottom:10,background:P.card,borderRadius:16,border:`1px solid ${P.border}`}}>
              <QueBlock hex={r.chinh} lv={r.lineValues} mv={r.moving} label="CHÁNH" w={80}/>
              {r.queHo&&<QueBlock hex={r.queHo} lv={[r.lineValues[1],r.lineValues[2],r.lineValues[3],r.lineValues[2],r.lineValues[3],r.lineValues[4]]} label="HỘ" w={70}/>}
              {r.bien&&<QueBlock hex={r.bien} lv={r.lineValues.map((v,i)=>r.moving.includes(i)?(v===1?0:1):v)} label="BIẾN" w={70}/>}
            </div>
          ...
          {/* Saved list modal */}
          {showSaved&&(
            <div onClick={()=>setShowSaved(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:16}}>
              <div onClick={e=>e.stopPropagation()} style={{background:P.card,borderRadius:16,padding:16,maxWidth:500,width:'100%',maxHeight:'80vh',color:P.fg}}>
                <h3 style={{margin:'0 0 12px',color:P.accent}}>📋 Quẻ Đã Lưu ({saved.length})</h3>
                <div style={{maxHeight:'60vh',overflowY:'auto'}}>
                  {saved.length===0&&<div style={{textAlign:'center',padding:20,color:P.muted}}>Chưa lưu quẻ nào</div>}
                  {saved.map(h=>(
                    <div key={h.id} style={{display:'flex',alignItems:'center',gap:8,padding:10,marginBottom:6,border:`1px solid ${P.border}`,borderRadius:10,background:P.bg}}>
                      <div style={{flex:1,cursor:'pointer'}} onClick={()=>{setQResult(h);setQName(h.name||'');setQText(h.question||'');setShowSaved(false)}}>
                        <div style={{fontWeight:600,color:P.accent,fontSize:14}}>{h.chinh?h.chinh[1]:''}{h.bien?' → '+shortQ(h.bien):''}</div>
                        <div style={{fontSize:11,color:P.muted,marginTop:2}}>{h.name||''} • {h.question||'—'} • {h.ts||''}</div>
                      </div>
                      <button onClick={()=>{const u=saved.filter(s=>s.id!==h.id);setSaved(u);localStorage.setItem('kd_saved',JSON.stringify(u))}} style={{background:'none',border:'none',color:P.muted,fontSize:16,cursor:'pointer'}}>✕</button>
                    </div>
                  ))}
                </div>
                <button onClick={()=>setShowSaved(false)} style={{width:'100%',marginTop:10,padding:10,background:P.accent,color:'#fff',border:'none',borderRadius:8,fontWeight:700,cursor:'pointer'}}>Đóng</button>
              </div>
            </div>
          )}
          <Popup2/>
  ...
  // ======== NGẪU NHIÊN (pull down to recast) ========
  if(view==='ngaunhien'&&result?.chinh){
    const bLv=result.bien?result.lineValues.map((v,i)=>result.moving.includes(i)?(v===1?0:1):v):null;
    const hoLv=result.queHo?[result.lineValues[1],result.lineValues[2],result.lineValues[3],result.lineValues[2],result.lineValues[3],result.lineValues[4]]:null;
    return(
      <div style={{...wrap,transform:`translateY(${pullY}px)`,transition:pullY===0?'transform .3s':'none'}} onTouchStart={e=>{onTS(e);onPullTS(e)}} onTouchMove={onPullTM} onTouchEnd={e=>{onTE(e);onPullTE()}}>
        {pullY>10&&<div style={{textAlign:'center',padding:8,fontSize:12,color:P.accent}}>{pullY>50?'↓ Thả để gieo lại':'↓ Kéo xuống để gieo lại'}</div>}
        <div style={{maxWidth:600,margin:'0 auto',padding:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <button onClick={()=>{setView('home');setResult(null)}} style={{background:'none',border:'none',color:P.accent,fontSize:13,cursor:'pointer',fontWeight:600}}>← Quay lại</button>
          ...
          <div style={{display:'flex',justifyContent:'space-evenly',padding:'28px 12px',marginBottom:10,background:P.card,borderRadius:16,border:`1px solid ${P.border}`}}>
            <QueBlock hex={result.chinh} lv={result.lineValues} mv={result.moving} label="CHÁNH" w={85}/>
            {result.queHo&&<QueBlock hex={result.queHo} lv={hoLv} label="HỘ" w={70}/>}
            {result.bien&&<QueBlock hex={result.bien} lv={bLv} label="BIẾN" w={70}/>}
          </div>
          ...
          <div style={{textAlign:'center',color:P.muted,fontSize:12,padding:8}}>Kéo màn hình xuống để gieo quẻ mới</div>

          <button onClick={()=>{setResult(null);setView('result')}} style={{display:'none'}}/>
        </div>
        <Settings2/><Popup2/>
      </div>
    );
    ...
    return(
      <div style={wrap} onTouchStart={onTS} onTouchEnd={onTE}>
        <div style={{maxWidth:480,margin:'0 auto',padding:20}}>
          ...
          <button onClick={()=>cast('dacbiet')} style={{width:'100%',padding:14,background:P.blue,color:'#fff',border:'none',borderRadius:12,fontWeight:700,cursor:'pointer',fontSize:16,marginBottom:16}}>Xem Quẻ</button>
          ...
          {result&&result.method==='Đặc Biệt'&&(
            <div style={{background:P.card,borderRadius:14,border:`1px solid ${P.border}`,padding:16,marginBottom:12}}>
              <div style={{display:'flex',justifyContent:'space-evenly',marginBottom:8}}>
                <QueBlock hex={result.chinh} lv={result.lineValues} mv={result.moving} label="CHÁNH" w={65}/>
                {result.queHo&&<QueBlock hex={result.queHo} lv={[result.lineValues[1],result.lineValues[2],result.lineValues[3],result.lineValues[2],result.lineValues[3],result.lineValues[4]]} label="HỘ" w={55}/>}
                {result.bien&&<QueBlock hex={result.bien} lv={result.lineValues.map((v,i)=>result.moving.includes(i)?(v===1?0:1):v)} label="BIẾN" w={55}/>}
              </div>
              <button onClick={()=>{setSpecialNum('');setResult(null)}} style={{width:'100%',padding:10,background:P.bg,border:`1px solid ${P.border}`,borderRadius:8,cursor:'pointer',color:P.blue,fontWeight:600,fontSize:13}}>Gieo tiếp số khác</button>
            </div>
          )}
        </div>
        <Popup2/>
    ...
    return(
      <div style={wrap} onTouchStart={onTS} onTouchEnd={onTE}>
        <div style={{maxWidth:480,margin:'0 auto',padding:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
            <button onClick={()=>{setView('home');setManualMoving([])}} style={{background:'none',border:'none',color:P.accent,fontSize:13,cursor:'pointer',fontWeight:600}}>← Quay lại</button>
  ...
  // ======== RESULT (generic - from Thời/Khắc/Giây/Nhập + AI) ========
  if(view==='result'&&result?.chinh){
    ...
    return(
      <div style={wrap} onTouchStart={onTS} onTouchEnd={onTE}>
        <div style={{maxWidth:600,margin:'0 auto',padding:16}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <button onClick={()=>{setView('home');setResult(null);setLuanResult('');setChatHistory([])}} style={{background:'none',border:'none',color:P.accent,fontSize:13,cursor:'pointer',fontWeight:600}}>← Quay lại</button>
          ...
          <div style={{display:'flex',justifyContent:'space-evenly',padding:'28px 12px',marginBottom:8,background:P.card,borderRadius:16,border:`1px solid ${P.border}`}}>
            <QueBlock hex={result.chinh} lv={result.lineValues} mv={result.moving} label="CHÁNH" w={90}/>
            {result.queHo&&<QueBlock hex={result.queHo} lv={hoLv} label="HỘ" w={75}/>}
            {result.bien&&<QueBlock hex={result.bien} lv={bLv} label="BIẾN" w={75}/>}
          </div>
          ...
          {(luanResult||chatHistory.length>0)&&(
            <div style={{border:`1px solid ${P.border}`,borderRadius:14,padding:14,marginBottom:12,background:P.card}}>
              ...
              {done&&(
                <div style={{display:'flex',gap:6,marginTop:10,borderTop:`1px solid ${P.border}`,paddingTop:10}}>
  ...
  return<div style={wrap}><div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100dvh'}}><button onClick={()=>setView('home')} style={{padding:'12px 24px',background:P.accent,color:'#fff',border:'none',borderRadius:10,cursor:'pointer',fontWeight:700}}>← Quay lại</button></div></div>;
}
