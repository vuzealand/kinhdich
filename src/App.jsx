...
export default function App(){
  ...
  // Popup
  const Pop=()=>{if(!popup)return null;const sumKey=popup[1].toUpperCase();const sum=QUE_SUMMARY[sumKey];return<div onClick={()=>setPopup(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,padding:16}}><div onClick={e=>e.stopPropagation()} style={{background:T.card,borderRadius:20,maxWidth:400,width:'100%',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,.2)',maxHeight:'80vh',display:'flex',flexDirection:'column'}}><div style={{background:`linear-gradient(135deg, ${T.accent}, ${dark?'#a08050':'#6b5530'})`,padding:'24px 28px',textAlign:'center'}}><div style={{fontSize:26,fontWeight:700,color:'#fff',fontFamily:"'Cormorant Garamond',Georgia,serif"}}>{popup[1]}</div></div><div style={{padding:'24px 28px',overflowY:'auto',flex:1}}>{sum&&<div style={{fontSize:17,fontWeight:700,color:T.red,marginBottom:14,fontFamily:"'Cormorant Garamond',Georgia,serif"}}>{sum[0]}.</div>}{sum&&<div style={{fontSize:14,lineHeight:1.8,color:T.fg,marginBottom:14}}>{sum[1]}</div>}{!sum&&<div style={{fontSize:14,lineHeight:1.8,color:T.fg,marginBottom:14}}>{popup[5]}</div>}<div style={{fontSize:12,color:T.muted,borderTop:`1px solid ${T.border}`,paddingTop:10}}>Thượng: {TRIGRAMS[popup[3]]?.name} ({TRIGRAMS[popup[3]]?.nature}) — Hạ: {TRIGRAMS[popup[4]]?.name} ({TRIGRAMS[popup[4]]?.nature})</div></div><button onClick={()=>setPopup(null)} style={{width:'100%',padding:16,background:T.accent,color:'#fff',border:'none',fontSize:15,fontWeight:600}}>Đóng</button></div></div>};
